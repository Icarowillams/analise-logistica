    ...
    <div className="p-3 bg-white rounded border space-y-2">
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center space-x-2">
          ...
          <label htmlFor={`perm-${label}`} className="text-xs">Permanente</label>
        </div>
        {!permanente && <Input type="date" value={prazo || ''} onChange={(e) => onChangePrazo(e.target.value)} className="h-8 text-xs flex-1" />}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="p-3 bg-white rounded border space-y-3">
      ...
      {(formData.tipo_exposicao === 'ponto_extra' || formData.tipo_exposicao === 'os_dois') && (
        <PrazoField label="Ponto Extra" prazo={formData.ponto_extra_prazo} permanente={formData.ponto_extra_permanente}
          onChangePrazo={(v) => update('ponto_extra_prazo', v)} onChangePermanente={(v) => update('ponto_extra_permanente', v)} />
      )}
      {(formData.tipo_exposicao === 'gondola' || formData.tipo_exposicao === 'os_dois') && (
        <PrazoField label="Gôndola" prazo={formData.gondola_prazo} permanente={formData.gondola_permanente}
          onChangePrazo={(v) => update('gondola_prazo', v)} onChangePermanente={(v) => update('gondola_permanente', v)} />
      )}
    </div>
  );
}