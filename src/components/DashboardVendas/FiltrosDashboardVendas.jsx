    ...
    <div className={`${compact ? 'bg-transparent rounded-none shadow-none p-0' : 'bg-white rounded-xl shadow-lg p-6'} space-y-4`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div>
          ...
          <Input type="text" placeholder="Digite o número" value={filtros.numPedido} onChange={(e) => setFiltros({ ...filtros, numPedido: e.target.value.replace(/\D/g, '') })} className="mt-1" />
        </div>
        <div className="md:col-span-2">
          <Label className="text-sm font-medium text-slate-700">Busca Geral</Label>
          <Input type="text" placeholder="Buscar em todos os campos..." value={filtros.busca} onChange={(e) => setFiltros({ ...filtros, busca: e.target.value })} className="mt-1" />
        </div>
      </div>
    </div>
  );
