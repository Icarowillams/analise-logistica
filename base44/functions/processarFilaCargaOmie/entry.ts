    ...
    try {
      ...
      if (data.faultstring) {
        ...
        if (res.status === 425 || msg.includes('consumo indevido') || msg.includes('bloqueada') || msg.includes('bloqueio')) {
          ...
          throw Object.assign(new Error(data.faultstring), { bloqueio: true });
        }
...
Deno.serve(async (req) => {
  try {
    ...
    // ═══ PASSO 2: TIMEOUT — Limpar itens travados em "processando" há mais de 3 minutos ═══
    ...
    if (!pendentes.length) {
      return Response.json({ sucesso: true, processados: 0, orfaos_limpos: orfaosLimpos, mensagem: orfaosLimpos > 0 ? `${orfaosLimpos} itens órfãos limpos` : 'Nenhum item pendente' });
    }
