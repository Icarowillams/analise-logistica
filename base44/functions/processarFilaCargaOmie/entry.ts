...
async function adquirirPortaoGlobal(base44, nome) {
  ...
  // Sob 429 o .catch([]) antigo fazia a função CRIAR um 2º registro → foi assim que o
  // órfão de 27/06 reapareceu. Agora, se a leitura falha, abortamos sem criar nada.
  let rows;
  try {
...
Deno.serve(async (req) => {
  try {
    ...
    if (!lock.adquirido) {
      // Lock ativo (TTL válido) = uma cadeia real está rodando AGORA e ela já vai
      // auto-encadear ao terminar (bloco AUTO-ENCADEAMENTO no fim). NÃO reagendamos aqui:
      // o reagendamento por setTimeout, multiplicado pelas N invocações concorrentes
      // (auto-encadeamento + scheduler + clique humano) que chegam enquanto o lock está
      // ativo, gerava uma TEMPESTADE de invocações ("Lock ativo" 8x por execução) que
      // estourava o rate limit do Base44 (429) e recriava duplicatas dos locks. A rede de
      // segurança contra cadeia morta é o TTL curto do lock (2min) + o scheduler periódico.
      console.log('[FILA CARGA] Lock ativo — outra cadeia já está processando. Saindo sem reagendar.');
