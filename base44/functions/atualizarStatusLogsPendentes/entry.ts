...
async function omieCallRaw(base44, endpoint, param, options = {}) {
  const { appKey, appSecret } = await getOmieCredentials(base44);
  const call = options.call || '';
  if (!appKey || !appSecret) throw new Error('Credenciais Omie não configuradas.');
  if (!call) throw new Error('Informe options.call com o método Omie.');
  const cb = await checkCircuitBreaker(base44);
  if (cb.blocked) throw Object.assign(new Error(`API Omie bloqueada até ${cb.blockedUntil}`), { bloqueio: true });
    ...
    try {
      ...
      if (data.faultstring) {
        ...
        if (res.status === 425 || msg.includes('consumo indevido') || msg.includes('bloqueada') || msg.includes('bloqueio')) {
          ...
          throw Object.assign(new Error(data.faultstring), { bloqueio: true, suspensao: true });
        }
...
async function omieCall(base44, call, param) {
  return omieCallRaw(base44, 'produtos/pedido/', param, { call });
}
...
async function consultarStatusReal(base44, codigoPedido, mockOmieResponse = null) {
  ...
  if (etapa === '60') {
    return { etapa, status_real: 'emitida', numero_nf: nNF || '', codigo_sefaz: '100', mensagem: nNF ? `NF ${nNF} autorizada` : 'NF emitida (etapa 60 confirmada no Omie)' };
  }
...
Deno.serve(async (req) => {
  try {
    ...
    const DELAY_ENTRE_CONSULTAS_MS = 3000;
    ...
    // Deduplicar logs por codigo_pedido
